...
(function () {
      ...
      <div class="card">
        ...
        <div class="break-wrap">
          ...
          <div class="break-stats">
            ...
            ${selId === 'hero' ? '<div class="row mt8"><button class="btn btn-sm btn-blue" data-act="usepill">使用渡劫丹(单次)</button><button class="btn btn-sm ' + (S.autoPill ? 'btn-green' : '') + '" data-act="autopill">' + (S.autoPill ? '自动使用·开' : '自动使用·关') + '</button>' + (S.singlePill ? '<span class="green"> 已装备</span>' : '') + '</div>' : ''}
          </div>
        </div>
      </div>
  ...
  const act = {
    ...
    'tribp': function (a) { const r = C.partnerTribulate(S.game, a, false); toast(r.msg); if (r.ok) { C.save(S.game); render(); } },
    'fld': function (a) {
      const g = S.game;

...
(function () {
  ...
  const S = {
    ...
    bulkDismiss: false, bulkSel: {}, partnerModal: null,
  ...
  function renderPartnerModal() {
    const old = document.getElementById('partner-modal');
    if (old) old.remove();
    if (!S.partnerModal) return;
    const g = S.game;
    const p = g.partners.find(x => x.iid === S.partnerModal);
    if (!p) { S.partnerModal = null; return; }
    const tpl = DATA.PARTNERS.find(x => x.id === p.pid);
    const st = C.unitStats(g, p.iid) || { atk: 0, hp: 0, def: 0, spd: 0 };
    const qc = qColor(tpl.q);
    const inF = g.formation.includes(p.iid), inJ = g.juling.includes(p.iid), inWx = g.wanxiang.includes(p.iid);
    function targetTxt(tg) { return { one: '单体', all: '全体', allylow: '友方最低', two: '两个目标', self: '自身' }[tg] || tg; }
    function typeTxt(ty) { return { dmg: '伤害', heal: '治疗', buff: '增益', debuff: '减益' }[ty] || ty; }
    const skHtml = (tpl.skills || []).map((sid, i) => {
      const sk = DATA.SKILLS[sid];
      if (!sk) return '';
      const label = i === 0 ? '普通技能' : (i === tpl.skills.length - 1 ? '专属技能' : '技能' + (i + 1));
      return '<div class="pub-skill"><b>' + label + ' · ' + sk.name + '</b>' +
        '<span class="dim"> ' + typeTxt(sk.type) + (sk.mult ? ' ' + (Math.round(sk.mult * 100)) + '%' : '') + ' · 目标：' + targetTxt(sk.target) + '</span>' +
        (sk.desc ? '<div class="dim">' + sk.desc + '</div>' : '') + '</div>';
    }).join('');
    const d = document.createElement('div');
    d.id = 'partner-modal';
    d.className = 'eq-modal';
    d.innerHTML = '<div class="eq-modal-card">' +
      '<div class="eq-modal-title" style="color:' + qc + '">' + tpl.name + ' <span class="tag">' + C.colorName(tpl.q) + '</span> · ★' + p.stars + '</div>' +
      '<div class="dim" style="text-align:center;margin-top:4px">' + ROLES[tpl.role] + ' · ' + C.partnerRealmLabel(p) + ' · ' + tpl.el + '属性</div>' +
      '<div class="eq-modal-stat">攻击 ' + F(st.atk) + ' · 生命 ' + Math.round(st.hp) + '<br>物防 ' + F(st.def) + ' · 法防 ' + F(st.def) + ' · 速度 ' + Math.round(st.spd) + '</div>' +
      '<div class="eq-modal-btns">' +
        '<button class="btn btn-sm ' + (inF ? 'btn-red' : 'btn-green') + '" data-act="fld" data-a="' + p.iid + '">' + (inF ? '下阵' : '上阵') + '</button>' +
        '<button class="btn btn-sm ' + (inJ ? 'btn-red' : 'btn-blue') + '" data-act="jld" data-a="' + p.iid + '">' + (inJ ? '移出聚灵' : '聚灵') + '</button>' +
        '<button class="btn btn-sm" data-act="tribp" data-a="' + p.iid + '">渡劫</button>' +
        '<button class="btn btn-sm btn-red" data-act="dismiss" data-a="' + p.iid + '">遣散</button>' +
      '</div>' +
      '<div class="skill-block">' + (skHtml || '<div class="dim">暂无技能。</div>') + '</div>' +
      '<div class="eq-modal-btns"><button class="btn btn-sm" data-act="pmclose">关闭</button></div>' +
      '</div>';
    document.body.appendChild(d);
  }
  ...
  function dimPartnerHtml(g) {
    ...
    const bd = S.bulkDismiss;
    // 批量遣散工具栏
    html += '<div class="eq-toolbar">';
    html += bd
      ? '<button class="btn btn-sm btn-red" data-act="bulkdismiss">退出批量</button>'
      : '<button class="btn btn-sm btn-red" data-act="bulkdismiss">批量遣散</button>';
    if (bd) {
      html += '<button class="btn btn-sm" data-act="bulkq" data-a="2">选蓝</button>';
      html += '<button class="btn btn-sm" data-act="bulkq" data-a="3">选紫</button>';
      html += '<button class="btn btn-sm" data-act="bulkq" data-a="4">选金</button>';
      html += '<button class="btn btn-sm" data-act="bulkclear">清空</button>';
    }
    html += '</div>';
    ...
    g.partners.forEach(p => {
      ...
      const qc = qColor(tpl.q);
      const inF = g.formation.includes(p.iid), inJ = g.juling.includes(p.iid), inWx = g.wanxiang.includes(p.iid);
      const inTeams = inF || inJ || inWx;
      if (bd) {
        const sel = S.bulkSel[p.iid];
        const dis = inTeams ? ' disabled' : '';
        html += '<div class="partner-card compact' + (sel ? ' sel' : '') + '"' + (inTeams ? '' : ' data-bulk="' + p.iid + '"') + '>' +
          '<input type="checkbox" data-bulksel="' + p.iid + '"' + (sel ? ' checked' : '') + dis + '>' +
          '<div class="p-cardmid"><b style="color:' + qc + '">' + tpl.name + '</b>' +
          '<small>' + C.partnerRealmLabel(p) + (inTeams ? '（上阵/聚灵/万象中，不可遣散）' : '') + '</small></div>' +
          '</div>';
      } else {
        html += '<div class="partner-card compact" data-act="opendetail" data-a="' + p.iid + '">' +
          '<div class="p-cardmid"><b style="color:' + qc + '">' + tpl.name + '</b>' +
          '<small>' + C.partnerRealmLabel(p) + '</small></div>' +
          '<div class="p-btncol"><button class="btn btn-sm btn-red" data-act="dismiss" data-a="' + p.iid + '">遣散</button></div>' +
          '</div>';
      }
    });
    if (bd) {
      const n = Object.keys(S.bulkSel).filter(k => S.bulkSel[k]).length;
      html += '<div class="eq-toolbar mt8"><button class="btn btn-sm btn-red" data-act="bulkdo"' + (n ? '' : ' disabled') + '>遣散选中（' + n + '）</button></div>';
      html += '<div class="dim mt8">勾选蓝色/紫色/金色道友可一键遣散（红卡请手动确认）；正在上阵/聚灵/万象中的道友不会被遣散。</div>';
    }
    html += '</div>';
    return html;
  }
...
const r = C.farmMainline(g, (line) => addLog('main', line.msg, 'bl-' + line.cls));
    ...
    'opendetail': function (a) { S.partnerModal = a; renderPartnerModal(); },
    'pmclose': function () { S.partnerModal = null; render(); },
    'bulkdismiss': function () {
      S.bulkDismiss = !S.bulkDismiss;
      if (!S.bulkDismiss) S.bulkSel = {};
      render();
    },
    'bulkq': function (a) {
      const q = +a || 0;
      ...
      g.partners.forEach(p => {
        const tpl = DATA.PARTNERS.find(x => x.id === p.pid);
        if (tpl.q !== q) return;
        if (g.formation.includes(p.iid) || g.juling.includes(p.iid) || g.wanxiang.includes(p.iid)) return;
        S.bulkSel[p.iid] = true;
      });
      render();
    },
    'bulkclear': function () { S.bulkSel = {}; render(); },
    'bulkdo': function () {
      const g = S.game;
      const ids = Object.keys(S.bulkSel).filter(k => S.bulkSel[k]);
      if (!ids.length) { toast('请先勾选要遣散的道友'); return; }
      if (!confirm('确定遣散选中的 ' + ids.length + ' 位道友吗？身上装备将退回背包。')) { toast('已取消'); return; }
      let n = 0;
      ids.forEach(iid => {
        if (g.formation.includes(iid) || g.juling.includes(iid) || g.wanxiang.includes(iid)) return;
        const r = C.dismissPartner(g, iid);
        if (r.ok) n++;
      });
      S.bulkSel = {}; S.bulkDismiss = false;
      C.save(g); toast('已遣散 ' + n + ' 位道友'); render();
    },
    ...
    'tribp': function (a) { const r = C.partnerTribulate(S.game, a, S.autoPill || S.singlePill); S.singlePill = false; toast(r.msg); if (r.ok) { C.save(S.game); render(); } if (S.partnerModal) renderPartnerModal(); },
    'dismiss': function (a) {
      const g = S.game;
      ...
      if (S.partnerModal) renderPartnerModal();
    },
    'fld': function (a) {
      const g = S.game;
...
g.formation.push(a);
  ...
  document.addEventListener('click', function (e) {
    ...
    if (bk) { e.preventDefault(); const id = bk.dataset.bulk; S.bulkSel[id] = !S.bulkSel[id]; render(); return; }
    // 上阵阵容点击选中/互换

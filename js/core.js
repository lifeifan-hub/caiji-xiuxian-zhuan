...
(function () {
  ...
  function normalizeState(state) {
    ...
    // 一个单位每个部位只保留一件穿戴装备（优先高品质）
    const seenSlots = {};
    (state.equipment || []).forEach(e => {
      if (state.equipped && state.equipped[e.iid]) {
        const unit = state.equipped[e.iid], q = QMAP[e.quality], k = unit + '/' + e.slot;
        const prev = seenSlots[k];
        if (!prev || q > prev.q) { if (prev) delete state.equipped[prev.iid]; seenSlots[k] = { iid: e.iid, q }; }
        else delete state.equipped[e.iid];
      }
    });
  ...
  function equipTotalBonus(state, key) {
    ...
    list.forEach(e => {
      ...
      if (P.SLOT_STAT[e.slot] === 'spd') bonus.spd += Math.round(b.flat.spd);
  ...
  function buildTimerDuration(target) {
    // 升到2级=60秒, 3级=1800秒, 4级=7200秒, 5级=14400, 6级=28800, 7级=57600, 以后每级×2
    if (target <= 2) return 60;
    if (target === 3) return 1800;
    return 7200 * Math.pow(2, target - 4);
  }
